package com.cofe.solution.ui.device.record.presenter;
...
//        endCalendar.set(Calendar.SECOND, 59);
     ...
     * @param selectedItems
     */
    public void deleteVideo(List<Integer> selectedItems) {
        if (recordManager instanceof CloudRecordManager) {
            for(int i = 0; i<selectedItems .size();  i++) {
                H264_DVR_FILE_DATA h264DvrFileData = recordList.get(selectedItems.get(i));
                ((CloudRecordManager) recordManager).deleteVideo(h264DvrFileData.fileName);
            }
        }
    }
